...
def map():
    ...
    data = pandas.read_csv("IPLAuction.csv")
    lat = list(data["Y"])
    lon = list(data["X"])
    ...
    for lt, ln in zip(lat,lon):
        fg.add_child(folium.CircleMarker(location=[lt,ln],popup="trending",radius = 4,fill=True,  color = 'red', fill_opacity=1.7,clustered_marker=True))
